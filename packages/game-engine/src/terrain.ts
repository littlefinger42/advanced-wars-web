...
export const TERRAIN_DEFENSE: Record<TerrainType | string, number> = {
  plain: 1,
  mountain: 4,
  woods: 2,
  river: 0,
...
export const MOVEMENT_COSTS: Record<
  TerrainType | string,
  Partial<Record<MovementType, number>>
> = {
...
const RAIN_MULTIPLIER: Partial<Record<MovementType, number>> = {
  treads: 2,
  wheels: 2,
  foot: 1,
  mech: 1,
...
export function getDefenseStars(terrain: TerrainType | string, property: PropertyType): number {
  if (property) {
...
export function canMoveOn(terrain: TerrainType | string, movementType: MovementType): boolean {
