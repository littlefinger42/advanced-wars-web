...
const WOODS_PAL: string[] = ['', '#166534', '#14532d', '#4d7c0f', '#78350f']
const MOUNTAIN_PAL: string[] = ['', '#78716c', '#57534e', '#d6d3d1', '#fafaf9']
...
function makeWoods(): SpriteGrid {
  const g = makePlain()
  fillCircle(g, 9, 9, 6, 1)
  fillCircle(g, 22, 10, 7, 1)
  fillCircle(g, 15, 20, 8, 1)
  fillCircle(g, 12, 10, 5, 2)
  fillCircle(g, 24, 12, 5, 2)
  fillCircle(g, 18, 22, 6, 2)
...
function makeMountain(): SpriteGrid {
  ...
  for (let y = 8; y < 30; y++) {
    ...
    fillRect(g, 16 - half, y, half * 2 + 1, 1, 1)
  }
  for (let y = 12; y < 30; y++) {
    const half = Math.floor((y - 12) * 0.7)
    fillRect(g, 8 - half, y, half * 2 + 1, 1, 2)
  }
  for (let y = 14; y < 30; y++) {
    const half = Math.floor((y - 14) * 0.6)
    fillRect(g, 24 - half, y, half * 2 + 1, 1, 2)
  }
...
export const TERRAIN_SPRITES: Record<string, { grid: SpriteGrid; pal: string[] }> = {
